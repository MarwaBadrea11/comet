...
export const userService = {
  ...
  /**
   * GET /user/email/:email — exact-match lookup (not the search page's data
   * source; SearchScreen uses searchService.globalSearch instead, which
   * partial-matches across name/username/email in one call).
   */
  searchByEmail: async (email: string): Promise<UserProfile | null> => {
    if (!email?.trim()) {
      throw new Error('Email cannot be empty')
    }
    const { data } = await api.get(`/user/email/${encodeURIComponent(email.trim())}`)
